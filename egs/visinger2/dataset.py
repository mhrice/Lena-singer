...
class BaseDataset(torch.utils.data.Dataset):
    def __init__(self, hparams, fileid_list_path):
        ...
        if hparams.data.n_speakers > 0:
            self.spk2id, self.utt2spk = self.get_spk_map(
                os.path.join(hparams.data.data_dir, "spk2id.json"),
...
class SingDataset(BaseDataset):
    ...
    def __filter__(self):
        ...
        for file_id in self.fileid_list:
            ...
            if (
                not os.path.exists(
                    os.path.join(self.label_dir, self.fileid_list[index] + ".lab")
                )
                or not os.path.exists(
                    os.path.join(self.dur_dir, self.fileid_list[index] + ".lab")
                )
                or not os.path.exists(
                    os.path.join(self.mel_dir, self.fileid_list[index] + ".npy")
                )
                or not os.path.exists(
                    os.path.join(self.pitch_dir, self.fileid_list[index] + ".npy")
                )
            ):
                _is_qualified = False
            if _is_qualified:
    ...
    def parse_label(self, pho, pitchid, dur, slur, gtdur):
        ...
        for index in range(len(pho.split())):
            phos.append(
                npu.symbol_converter.ttsing_phone_to_int[pho.strip().split()[index]]
            )
            pitchs.append(
                npu.symbol_converter.ttsing_opencpop_pitch_to_int[
                    pitchid.strip().split()[index]
                ]
            )
        ...
        acc_duration = np.ceil(
            np.cumsum(gtdurs) / (self.hps.data.hop_size / self.hps.data.sample_rate)
        )
    ...
    def __getitem__(self, index):
            ...
            if mel.shape[0] > sum_dur:
                ...
                mel = np.concatenate(
                    [
                        mel,
                        mel.min()
                        * np.ones([sum_dur - mel.shape[0], self.hps.data.acoustic_dim]),
                    ],
                    axis=0,
                )
        ...
        f0 = np.load(
            os.path.join(self.f0_dir, self.fileid_list[index] + ".npy")
        ).reshape([-1])
        ...
        wav = load_wav(
            os.path.join(self.wav_dir, self.fileid_list[index] + ".wav"),
        ...
        if wav.shape[0] != sum_dur * self.hparams.data.hop_size:
            if (
                abs(wav.shape[0] - sum_dur * self.hparams.data.hop_size)
                > 40 * self.hparams.data.hop_size
            ):
            ...
            if wav.shape[0] > sum_dur * self.hparams.data.hop_size:
                ...
                wav = np.concatenate(
                    [
                        wav,
                        np.zeros([sum_dur * self.hparams.data.hop_size - wav.shape[0]]),
                    ],
                    axis=0,
                )
...
class SingCollate:
    ...
    def __call__(self, batch):
        ...
        mel_padded = torch.FloatTensor(
            len(batch), self.hparams.data.acoustic_dim, max_mel_len
        )
...
class DatasetConstructor:
    ...
    def _init_datasets(self):
        self._train_dataset = self.dataset_function[self.hparams.data.dataset_type](
            self.hparams,
            self.hparams.data.data_dir,
            self.hparams.data.training_filelist,
            self.hparams.data.training_labellist,
        )
        self._valid_dataset = self.dataset_function[self.hparams.data.dataset_type](
            self.hparams,
            self.hparams.data.data_dir,
            self.hparams.data.validation_filelist,
            self.hparams.data.validation_labellist,
        )

    def _init_collate(self):
        self._collate_fn = self.collate_function[self.hparams.data.collate_type](
            self.hparams
        )
