...
def pad_v2(input_ele: List[torch.Tensor], mel_max_length: int = 0):
    if mel_max_length:
...
class LengthRegulator(nn.Module):
    ...
    def LR(self, x, duration, max_len):
        ...
        return output, torch.tensor(mel_len)
...
class ConvReluNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(1, self.n_layers):
            conv: ConvModuleInterface = self.conv_layers[i]
            norm: NormModuleInterface = self.norm_layers[i]
            x_ = conv.forward(x)
            x_ = norm.forward(x_)
...
@torch.jit.interface
class ConvModuleInterface(nn.Module):
    def forward(self, input) -> torch.Tensor:
        pass


@torch.jit.interface
class NormModuleInterface(nn.Module):
...
class Generator_Harm(torch.nn.Module):
    def __init__(self, hps):
        ...
        self.prenet = Conv1d(
            hps.model.hidden_channels, hps.model.hidden_channels, 3, padding=1
        )
        ...
        #    bidirectional=True)
        self.postnet = Conv1d(
            hps.model.hidden_channels, hps.model.n_harmonic + 1, 3, padding=1
        )
...
class Generator(torch.nn.Module):
    def __init__(
        ...
        self.concat_pre = Conv1d(
            upsample_initial_channel + hps.model.n_harmonic + 2,
            upsample_initial_channel,
            3,
            1,
            padding=1,
        ...
        for i in range(len(upsample_rates)):
            ...
            self.concat_conv.append(
                Conv1d(ch + hps.model.n_harmonic + 2, ch, 3, 1, padding=1, bias=False)
            )
...
class Generator_Noise(torch.nn.Module):
    def __init__(self, hps):
        ...
        self.istft_amplitude = torch.nn.Conv1d(
            hps.model.hidden_channels, self.fft_size // 2 + 1, 1, 1
        )
    ...
    def __init__(self, in_channels, hidden_channels=512):
        ...
        self.discriminator += [
            ...
            nn.Sequential(
                ...
                nn.ReflectionPad2d((1, 1, 1, 1)),
                nn.utils.weight_norm(
                    nn.Conv2d(hidden_channels, 1, kernel_size=(3, 3), stride=(1, 1))
                ),
            ),
        ]
...
class SynthesizerTrn(nn.Module):
    ...
    def __init__(self, hps):
        ...
        self.energy_prenet = nn.Conv1d(
            1, hps.model.prior_hidden_channels + 2, 3, padding=1
        )
        self.mel_prenet = nn.Conv1d(
            hps.data.acoustic_dim, hps.model.prior_hidden_channels + 2, 3, padding=1
        )
    ...
    def forward(
        ...
        LF0 = 2595.0 * torch.log10(1.0 + F0 / 700.0)
        LF0 = LF0 / 500

        # aam
        predict_lf0, predict_bn_mask = self.f0_decoder(
            decoder_input + decoder_input_pitch, bn_lengths, spk_emb=g
        )
        predict_mel, predict_bn_mask = self.mel_decoder(
            decoder_input + self.f0_prenet(LF0), bn_lengths, spk_emb=g
        )

        predict_energy = (
            predict_mel.detach().sum(1).unsqueeze(1) / self.hps.data.acoustic_dim
        )
        ...
        decoder_output, predict_bn_mask = self.decoder(
            decoder_input, bn_lengths, spk_emb=g
        )
        ...
        x_slice, ids_slice = commons.rand_slice_segments(
            p_z, bn_lengths, self.hps.train.segment_size // self.hps.data.hop_size
        )
        F0_slice = commons.slice_segments(
            F0, ids_slice, self.hps.train.segment_size // self.hps.data.hop_size
        )
        dsp_slice = commons.slice_segments(
            dsp_o, ids_slice * self.hps.data.hop_size, self.hps.train.segment_size
        )
        condition_slice = commons.slice_segments(
            decoder_condition,
            ids_slice * self.hps.data.hop_size,
    ...
    def infer(
        ...
        # aam
        predict_lf0, predict_bn_mask = self.f0_decoder(
            decoder_input + decoder_input_pitch, y_lengths, spk_emb=g
        )
        predict_mel, predict_bn_mask = self.mel_decoder(
            decoder_input + self.f0_prenet(predict_lf0), y_lengths, spk_emb=g
        )
        ...
        # Apply random detuning per note
        for note in range(F0.shape[2]):
            if random.random() < random_detune_prob:
                freq_options = [
                    F0[:, :, note],
                    F0[:, :, note] * 0.943874 * 0.943874,
                    F0[:, :, note] * 0.943874,
                    F0[:, :, note] * 1.059463,
                    F0[:, :, note] * 1.059463 * 1.059463,
                ]
                if random_detune_prob == 1:
                    # Never pick correct note
                    freq_options = freq_options[1:-1]
                discrete = torch.distributions.Categorical(
                    torch.ones(len(freq_options))
                )
                F0[:, :, note] = freq_options[discrete.sample().item()]
