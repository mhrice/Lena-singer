...
class MultiHeadAttention(nn.Module):
    ...
    def attention(self, query, key, value, mask: torch.Tensor = None):
        ...
        kk = key.size()
        b, d, t_s, t_t = (kk[0], kk[1], kk[2], query.size(2))
    ...
    def _get_relative_embeddings(self, relative_embeddings, length: int):
        ...
        if pad_length > 0:
            padded_relative_embeddings = F.pad(
                ...
                #     torch.Tensor([[0, 0], [pad_length, pad_length], [0, 0]])
                # )
...
class FFN(nn.Module):
    ...
    def forward(self, x, x_mask):
        ...
        if self.causal:
            x = self.conv_1(self._causal_padding(x * x_mask))
        else:
        ...
        if self.causal:
            x = self.conv_2(self._causal_padding(x * x_mask))
        else:
